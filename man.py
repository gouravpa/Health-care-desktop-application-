...
class Hospital:
    ...
    def AdminMain(self):
        ...
        self.b6=Button(self.f1,text="Admin Register",bg="light grey",fg="black",font="elephant 10",command=lambda:self.Register())
        self.b6.place(x=1150,y=20)
        self.b7=Button(self.f1,text="Patient Register",bg="light grey",fg="black",font="elephant 10",command=lambda:self.patient(x))
        self.b7.place(x=1370,y=20)
    ...
    def pRegister(self):
        self.t.destroy()
        self.t=Tk()
        self.t.title("Patient Registration Form")
        self.t.geometry("1366x768")
        self.t.iconbitmap("download.ico")
        self.loginf1=Frame(self.t,height=150,width=1366)
        self.logo=PhotoImage(file="mann.png")
        ...
        self.b=Button(self.loginf1,text="HOME",command=lambda:self.Main())
        self. b.place(x=1000,y=140)
        self.l=Label(self.loginf2,text="Patient Registration Form",font=("Elephant 30"))
        self.l.place(x=120,y=50)
        self.l1=Label(self.loginf2,text="First Name",font=("Elephant"))
        self.l1.place(x=100,y=130)
        self.fn=Entry(self.loginf2)
        ...
        self.ln.place(x=220,y=160)
        self.l1=Label(self.loginf2,text="Gender",font=("Elephant "))
        self.l1.place(x=100,y=190)
        self.ad=Entry(self.loginf2)
        self.ad.place(x=220,y=190)
        self.l1=Label(self.loginf2,text="Age",font=("Elephant"))
        self.l1.place(x=100,y=220)
        self.ag=Entry(self.loginf2)
        self.ag.place(x=220,y=220)
        self.l1=Label(self.loginf2,text="Email.",font=("Elephant"))
        self.l1.place(x=100,y=250)
        self.mn=Entry(self.loginf2)
        self.mn.place(x=220,y=250)
        self.l1=Label(self.loginf2,text="Mobile No",font=("Elephant"))
        self.l1.place(x=100,y=280)
        self.g=Entry(self.loginf2)
        self.g.place(x=220,y=280)
        self.l1=Label(self.loginf2,text="Address",font=("Elephant"))
        self.l1.place(x=100,y=310)
        self.d=Entry(self.loginf2)
        self.d.place(x=220,y=310)
        self.l1=Label(self.loginf2,text="Doctor Name",font=("Elephant"))
        self.l1.place(x=100,y=340)
        self.e=Entry(self.loginf2)
        self.e.place(x=250,y=340)
        self.l1=Label(self.loginf2,text="Disease",font=("Elephant"))
        self.l1.place(x=100,y=370)
        self.f=Entry(self.loginf2)
        self.f.place(x=220,y=380)
        self.b3=Button(self.loginf2,text="Register",font=("Elephant"),width=8,command=lambda:self.patientr())
        self.b3.place(x=160,y=430)

    def patientres(self):
        self.first=self.fn.get()
        self.last=self.ln.get()
        self.gender=self.ad.get()
        self.age=self.ag.get()
        self.email=self.mn.get()
        self.mobile=self.g.get()
        self.address=self.d.get()
        self.doctor=self.e.get()
        self.disease=self.f.get()

        return self.first,self.last,self.gender,self.age,self.email,self.mobile,self.address,self.doctor,self.disease
    ...
    def patientregistration(self,x):
        ...
        self.cur=self.con.cursor()
        try:
            self.cur.execute("create table demo(first varchar(50) not null,last varchar(50) not null,gender varchar(50) not null,age varchar(50),email varchar(50) not null,mob varchar(50) not null,address varchar(50) not null,doctor varchar(50) not null,,disease varchar(50))")


        except:
            pass
        x=self.cur.execute("select count(*) from  demo where first=%r and doctor=%r "%(self.credreg[0],self.credreg[8]))
        if list(x)[0][0]==0:
            if self.credreg[0]=="" or self.credreg[1]=="" or self.credreg[2]=="" or self.credreg[3]=="" or self.credreg[5]=="" or self.credreg[6]=="" or self.credreg[7]=="" or self.credreg[8]=="":
                messagebox.showinfo("Register","Empty Entry is not Allowed(except Email)")
            else:
                self.cur.execute("insert into  demo values(%r,%r,%r,%r,%r,%r,%r,%r,%r)"%(self.credreg[0],self.credreg[1],self.credreg[2],self.credreg[3],self.credreg[4],self.credreg[5],self.credreg[6],self.credreg[7],self.credreg[8]))
                self.con.commit()
    ...
    def patientreg(self):
        self.credreg=self.patientres()
        self.con=connect("manorama.db")
        self.cur=self.con.cursor()
        try:
            self.cur.execute("create table patient(first varchar(50) not null,last varchar(50) not null,gender varchar(50) not null,age varchar(50),email varchar(50) not null,mob varchar(50) not null,address varchar(50) not null,doctor varchar(50) not null,disease varchar(50) not null)")
        except:
            pass
        x=self.cur.execute("select count(*) from patient where first=%r and last=%r "%(self.credreg[0],self.credreg[1]))
        if list(x)[0][0]==0:
            if self.credreg[0]=="" or self.credreg[1]=="" or self.credreg[2]=="" or self.credreg[3]=="" or self.credreg[5]=="" or self.credreg[6]=="" or self.credreg[7]=="" or self.credreg[8]=="":
                messagebox.showinfo("Register","Empty Entry is not Allowed")
            else:
                self.cur.execute("insert into patient values(%r,%r,%r,%r,%r,%r,%r,%r,%r)"%(self.credreg[0],self.credreg[1],self.credreg[2],self.credreg[3],self.credreg[4],self.credreg[5],self.credreg[6], self.credreg[7],self.credreg[8]))
                self.con.commit()
                messagebox.showinfo("Register","Dear Patient you are Successelfully Registered")
                self.AdminMain()
        else:
    ...
    def patient(self,x):
        ...
        def create(self,x):
            ...
            self.c.execute("CREATE TABLE patient(sno integer primary key autoincrement,cust_name TEXT,User_name TEXT, mob_number integer,email_id TEXT,address TEXT,mc_model TEXT,mc_model1 TEXT,mc_qua integer,mc_qua1 integer, insert_date DATE DEFAULT CURRENT_DATE NOT NULL)")
            self.conn.commit()
            self.conn.close()
            self.create(self,x)

        self.label=Label(self.t,text="Customer Name",font=('arial',13,'bold'))
        self.label.place(x=30,y=60)

        self.name_entry = StringVar()
        self.name_entry = ttk.Entry(self.window,textvariable=self.name_entry)

        self.name_entery=ttk.Entry(self.window)
        self.name_entry.place(x=170,y=60,width=200,height=25)
        
        self.name_entry.focus()
        
        self.label1=Label(self.window,text="User Name",font=('arial',13,'bold'))
        self.label1.place(x=30,y=100)

        self.name_entry2=StringVar()
        self.name_entry2=ttk.Entry(self.window,textvariable=self.name_entry2)
        self.name_entry2=ttk.Entry(self.window)
        self.name_entry2.place(x=170,y=100,width=200,height=25)

        self.label=Label(self.window,text="Mobile Number",font=('arial',13,'bold'))
        self.label.place(x=30,y=140)

        self.name_entry3=IntVar()
        self.name_entry3=ttk.Entry(self.window,textvariable=self.name_entry3)
        self.name_entry3=ttk.Entry(self.window)
        self.name_entry3.place(x=170,y=140,width=200,height=25)

        self.label1=Label(self.window,text="Email Id",font=('arial',13,'bold'))
        self.label1.place(x=30,y=180)

        self.name_entry4=StringVar()
        self.name_entry4=ttk.Entry(self.window,textvariable=self.name_entry4)
        self.name_entry4=ttk.Entry(self.window)
        self.name_entry4.place(x=170,y=180,width=200,height=25)

        self.label1=Label(self.window,text="Address",font=('arial',13,'bold'))
        self.label1.place(x=30,y=220)

        self.name_entry5=StringVar()
        self.name_entry5=ttk.Entry(self.window,textvariable=self.name_entry5)
        self.name_entry5=ttk.Entry(self.window)
        self.name_entry5.place(x=170,y=220,width=200,height=25)
        
        self.label1=Label(self.window,text="Machine Model",font=('arial',13,'bold'))
        self.label1.place(x=30,y=260)

        self.name_entry6=StringVar()
        self.name_entry6=ttk.Entry(self.window,textvariable=self.name_entry6)
        self.name_entry6=ttk.Entry(self.window)
        self.name_entry6.place(x=170,y=260,width=200,height=25)
        
        self.name_entry7=StringVar()
        self.name_entry7=ttk.Entry(self.window,textvariable=self.name_entry7)
        self.name_entry7=ttk.Entry(self.window)
        self.name_entry7.place(x=170,y=290,width=200,height=25)

        self.label1=Label(self.window,text="Machine Quantity",font=('arial',13,'bold'))
        self.label1.place(x=30,y=330)

        self.name_entry8=StringVar()
        self.name_entry8=ttk.Entry(self.window,textvariable=self.name_entry8)
        self.name_entry8=ttk.Entry(self.window)
        self.name_entry8.place(x=170,y=330,width=99,height=25)
        
        self.name_entry9=StringVar()
        self.name_entry9=ttk.Entry(self.window,textvariable=self.name_entry9)
        self.name_entry9=ttk.Entry(self.window)
        self.name_entry9.place(x=270,y=330,width=100,height=25)

        def savedata():
            self.conn=sqlite3.connect('database.db')
            self.c=self.conn.cursor()
            self.c.execute('INSERT INTO timewatchcl(cust_name,user_name,mob_number,email_id,address,mc_model,mc_model1,mc_qua,mc_qua1) VALUES (?,?,?,?,?,?,?,?,?)',(self.name_entry.get(),self.name_entry2.get(),self.name_entry3.get(),self.name_entry4.get(),self.name_entry5.get(),self.name_entry6.get(),self.name_entry7.get(),self.name_entry8.get(),self.name_entry9.get()))
            self.conn.commit()
            print("saved")
    
        self.btn=ttk.Button(self.window,text="SAVE DATA",command=savedata)
        self.btn.place(x=170,y=380,width=200,height=30)
